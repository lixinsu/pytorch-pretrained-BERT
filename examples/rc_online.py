...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (i, entry) in enumerate(nbest):
            ...
            output["text"] = ''.join(entry.text.split())
...
def do_extract():
    ...
    passages = request.form['passages'].split('||||')
    print(passages)
    print(type(passages))
    # 数据封装
    eval_examples = []
    ...
    _, pid2nbest = write_predictions(eval_examples, eval_features, all_results,
                      args.n_best_size, args.max_answer_length,
                      args.do_lower_case, args.verbose_logging)
    pid2best = {k:v[0] for k,v in pid2nbest.items()}
    return jsonify(pid2best)
